...
```
HASH=$(docker run --rm ubuntu:24.04 bash -c "apt-get update -qq && apt-get install -y -qq openssl > /dev/null 2>&1 && openssl passwd -6 'TYPE_YOUR_PASSWD'")
echo "machine_pwd_hashed = \"$HASH\"" > secrets.pkrvars.hcl
```
...
```
packer init ubuntu
packer validate -var-file secrets.pkrvars.hcl ubuntu 
packer build -var-file secrets.pkrvars.hcl ubuntu
```
...
You need to boot it with UEFI:
```
qemu-system-x86_64 \                                                                                                      Mar/08-16:10:32
  -name ubuntu-qcow2 \
  -m 2048 \
  -smp 2 \
  -drive file=builds/qemu/ubuntu-server.qcow2,format=qcow2 \
  -drive if=pflash,format=raw,readonly=on,file=/opt/homebrew/share/qemu/edk2-x86_64-code.fd \
  -drive if=pflash,format=raw,file=builds/qemu/efivars.fd \
...
Make sure there's no other vagrant box with ubuntu-server name by doing: `vagrant box remove ubuntu-server`
