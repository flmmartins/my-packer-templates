...
Generate [Vagrant Boxes](http://vagrantup.com) and [QEMU images](https://www.qemu.org/) using [Packer](http://.packer.io). 

**You will also see virtualbox here but that is still a WIP and not functional**

# Packer How To

Install the tools from the links above

**Important:** At the moment virtualbox+packer doesn't work with Mac M1. See below on how to run only on qemu
...
```
packer build -only=source.qemu.install ubuntu
```

**Build images only from virtualbox:**
```
packer build -only=source.virtualbox-iso.install ubuntu
```
...
I don't work with Vagrant on a daily basis and here are some interesting I did on the past.
